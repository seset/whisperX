...
def cli():
    ...
    parser.add_argument("--language", type=str, default="ja", choices=sorted(LANGUAGES.keys()) + sorted([k.title() for k in TO_LANGUAGE_CODE.keys()]), help="language spoken in the audio, specify None to perform language detection")

    # alignment params
    parser.add_argument("--align_model", default=None, help="Name of phoneme-level ASR model to do alignment")
    parser.add_argument("--interpolate_method", default="nearest", choices=["nearest", "linear", "ignore"], help="For word .srt, method to assign timestamps to non-aligned words, or merge them into neighbouring.")
    parser.add_argument("--no_align", action='store_true', default=True, help="Do not perform phoneme alignment")
    ...
    parser.add_argument("--diarize_model", default="speaker-diarization-community-1", type=str, help="Name of the speaker diarization model to use")
    ...
    parser.add_argument("--best_of", type=optional_int, default=5, help="5, number of candidates when sampling with non-zero temperature")
    parser.add_argument("--beam_size", type=optional_int, default=3, help="8 slow but accurate,3 fast. only applicable when temperature is zero")
    parser.add_argument("--patience", type=float, default=1, help="1 optional patience value to use in beam decoding, as in https://arxiv.org/abs/2204.05424, the default (1.0) is equivalent to conventional beam search")
    ...
    parser.add_argument("--no_speech_threshold", type=optional_float, default=0.9, help="if the probability of the <|nospeech|> token is higher than this value AND the decoding has failed due to `logprob_threshold`, consider the segment as silence")
    ...
    parser.add_argument("--hf_token", type=str, default=None, help="token for accessing HuggingFace models (needed for some models such as large-v2)")
